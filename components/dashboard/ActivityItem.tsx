...
export function ActivityItem({ title, date, status, amount, id }: ActivityItemProps) {
  ...
  return (
    <div className="flex items-center justify-between p-6 bg-white rounded-[1.5rem] border border-gray-100 hover:border-purple-100 transition-all shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 font-medium mt-1">{format(new Date(date), "dd MMMM, yyyy")}</p>
        </div>
      </div>
      
      <div className="flex flex-col items-end gap-2">
         <span className="font-black text-gray-900 text-xl">₦ {(amount || 0).toLocaleString()}</span>
         <div className={cn(
