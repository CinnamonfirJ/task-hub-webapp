...
export function MainCategoryModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const slug = formData.name || formData.displayName.toLowerCase().replace(/\s+/g, "-");
      ...
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-8 border-b border-gray-50 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">{category ? "Edit" : "New"} Main Category</h2>
              <p className="text-xs text-gray-500 font-medium">Top-level service cluster</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-2">
            <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Display Name</label>
            <Input
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Internal Name / Slug</label>
            <Input
              placeholder="e.g. local-services"
              value={formData.name}
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50/50 border border-gray-100 rounded-2xl">
            <div>
              ...
              <p className="text-[10px] text-gray-500 font-medium">Visible to users when posting tasks</p>
            </div>
            <Switch
          ...
          <div className="pt-4 flex items-center justify-end gap-3">
            <Button type="button" variant="ghost" onClick={onClose} className="rounded-xl h-11 px-6 font-bold text-gray-500">
              Cancel
            </Button>
            <Button
              type="submit"
