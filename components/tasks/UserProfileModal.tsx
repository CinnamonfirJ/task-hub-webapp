...
export function UserProfileModal({ isOpen, onClose, user }: UserProfileModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='sm:max-w-[420px] max-h-[90vh] p-0 overflow-y-auto border-none rounded-[2.5rem]  bg-white focus:outline-none [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]'>
        {/* Banner with Gradient */}
        <div className='relative h-36 bg-linear-to-br from-[#7C3AED] via-[#6D28D9] to-[#4C1D95]'>
          {/* Abstract pattern overlay */}
          <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>

          <div className='absolute -bottom-14 left-8 p-1.5 bg-white rounded-[2rem] '>
          ...
          <div className='space-y-4'>
            <div className='flex items-center justify-between p-5 bg-purple-50/30 rounded-[2rem] border border-purple-100/50'>
              <div className='flex items-center gap-4'>
                <div className='w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-amber-400  border border-amber-50'>
              ...
              <div className='flex items-center gap-4'>
                <div className='w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-blue-500  border border-blue-50'>
                  <Calendar size={24} />
                </div>
                <div>
