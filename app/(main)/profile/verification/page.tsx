    ...
    <div className="p-4 md:p-8 max-w-2xl mx-auto space-y-10 pb-20">
      ...
      <div className="flex flex-col items-center text-center space-y-8">
        {isLoading ? (
          ...
          <>
            <div className={`p-8 rounded-full ${isVerified ? "bg-green-50" : "bg-orange-50"}`}>
               {isVerified ? (
            ...
            {!isVerified && (
              <div className="w-full pt-4">
                <Link href="/complete-profile">
                  <Button className="w-full bg-[#6B46C1] hover:bg-[#553C9A] py-8 text-lg font-bold rounded-2xl shadow-lg shadow-purple-200">
                    Start Verification
                  </Button>
                </Link>
              </div>
            ...
            <div className="w-full space-y-4 pt-8">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest text-left px-2">Why verify?</h3>
...
function VerificationBenefit({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
