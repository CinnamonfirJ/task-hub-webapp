        ...
        {taskMetrics.map((metric, idx) => (
          <Card key={idx} className="border border-gray-100 shadow-sm">
            <CardContent className="p-4">
              ...
              <div className={`text-[10px] mt-1 font-medium ${metric.color || "text-gray-400"}`}>
                {metric.label}
              </div>
            </CardContent>
          </Card>
        ...
        <CardContent className="p-0">
          <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
              {["All", "Open", "In-progress", "Completed", "Cancelled"].map((filter) => (
                <button
                  key={filter}
              ...
              <tbody className="divide-y">
                {tasks.map((task) => (
                  <tr key={task.id} className="group hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-5 font-bold text-gray-900">{task.title}</td>
                    <td className="px-6 py-5 text-gray-500 font-medium">{task.postedBy}</td>
                    <td className="px-6 py-5">
                      <span className="text-[10px] bg-gray-50 text-gray-600 px-2 py-1 rounded-md border border-gray-100 font-medium">
                        {task.category}
                      </span>
                    </td>
                    <td className="px-6 py-5 font-bold text-gray-900">{task.budget}</td>
                    <td className="px-6 py-5">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${task.statusColor}`}>
                        {task.status}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-gray-500 font-medium">{task.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
