...
import { useAdminWithdrawals, useApproveWithdrawal, useRejectWithdrawal, useCompleteWithdrawal } from "@/hooks/useAdminWithdrawals";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Loader2, CheckCircle, XCircle, Clock, Banknote, User, LayoutDashboard, Search, Filter } from "lucide-react";
...
export default function AdminWithdrawalsPage() {
  ...
  const { data: withdrawals, isLoading } = useAdminWithdrawals({ status: statusFilter });
    ...
    <div className="p-8 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">Monitor and process tasker withdrawal requests.</p>
        </div>
      </div>
                    ...
                    <tr key={item._id} className="bg-white hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-bold text-gray-900">{item.tasker?.fullName}</p>
                            <p className="text-[10px] text-gray-500">{item.tasker?.email}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <p className="font-medium text-gray-700">{item.bankName}</p>
                        <p className="text-xs text-gray-500 font-mono tracking-tighter">{item.accountNumber}</p>
                      </td>
